...
@Component({
  ...
  standalone: true,
  template: `
    <h1 class="text-3xl font-extrabold text-surface-900 tracking-tight mb-2">Animation</h1>
    ...
    <!-- Principles -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-4">Principles</h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        @for (p of principles; track p.title) {
          <div class="rounded-xl border border-surface-200 bg-white p-5">
            <p class="text-sm font-bold text-surface-900 mb-1">{{ p.title }}</p>
            <p class="text-sm text-surface-500 leading-relaxed">{{ p.description }}</p>
          </div>
        }
      </div>
    </section>
    ...
    <!-- Timing Reference -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-2">Timing Reference</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Duration -->
        <div>
          <p class="text-xs font-semibold text-surface-400 uppercase tracking-wider mb-3">Duration</p>
          <div class="rounded-xl border border-surface-200 bg-white overflow-hidden divide-y divide-surface-100">
            <div class="flex items-center gap-4 px-4 py-2 bg-surface-50">
              <span class="w-28 shrink-0 text-xs font-semibold text-surface-500 uppercase tracking-wider">Token</span>
              <span class="w-14 shrink-0 text-xs font-semibold text-surface-500 uppercase tracking-wider">Value</span>
              <span class="text-xs font-semibold text-surface-500 uppercase tracking-wider">Use for</span>
            </div>
            @for (row of durations; track row.token) {
              <div class="flex items-center gap-4 px-4 py-3">
                <span class="w-28 shrink-0 text-xs font-mono text-primary-600">{{ row.token }}</span>
                <span class="w-14 shrink-0 text-xs font-mono text-surface-400">{{ row.value }}</span>
                <span class="text-xs text-surface-500">{{ row.use }}</span>
              </div>
            }
          </div>
        </div>
        ...
        <!-- Easing -->
        <div>
          <p class="text-xs font-semibold text-surface-400 uppercase tracking-wider mb-3">Easing</p>
          <div class="rounded-xl border border-surface-200 bg-white overflow-hidden divide-y divide-surface-100">
            <div class="flex items-center gap-4 px-4 py-2 bg-surface-50">
              <span class="w-28 shrink-0 text-xs font-semibold text-surface-500 uppercase tracking-wider">Token</span>
              <span class="text-xs font-semibold text-surface-500 uppercase tracking-wider">Use for</span>
    ...
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-2">Tailwind Transitions</h2>
      <p class="text-sm text-surface-500 mb-6">
        Use
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">transition-*</code>
        utilities for hover and focus state changes. Always pair with
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">duration-*</code>
        and
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">ease-*</code>.
      </p>
      <div class="space-y-4">
        <!-- transition-colors -->
        <div class="rounded-xl border border-surface-200 bg-white p-5">
          <div class="flex items-start justify-between gap-4 mb-4">
            <div>
              <p class="text-sm font-bold text-surface-900 mb-0.5">Color</p>
              <p class="text-xs text-surface-500">
                Hover state changes on interactive elements — buttons, links, nav items.
              </p>
            </div>
            <code class="shrink-0 text-xs font-mono bg-surface-100 text-surface-700 px-2 py-1 rounded-md"
              >transition-colors duration-150 ease-out</code
            >
          </div>
          <div class="flex flex-wrap gap-3 items-center">
            <button
              class="px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-lg transition-colors duration-150 ease-out hover:bg-primary-600 active:bg-primary-700"
            >
              Save workout
            </button>
            <button
              class="px-4 py-2 bg-white border border-surface-200 text-surface-700 text-sm font-medium rounded-lg transition-colors duration-150 ease-out hover:bg-surface-100"
            >
              Cancel
            </button>
            <a
              class="px-3 py-2 text-sm font-medium text-surface-600 rounded-lg cursor-pointer transition-colors duration-150 ease-out hover:bg-surface-100 hover:text-surface-900"
            >
              Nav link
            </a>
          </div>
        ...
        <div class="rounded-xl border border-surface-200 bg-white p-5">
          <div class="flex items-start justify-between gap-4 mb-4">
            <div>
              <p class="text-sm font-bold text-surface-900 mb-0.5">Opacity</p>
              <p class="text-xs text-surface-500">
          ...
          <div class="flex flex-wrap gap-4 items-center">
            <button
              class="flex items-center gap-1.5 text-sm text-surface-500 transition-opacity duration-150 ease-out hover:opacity-50"
            >
        ...
        <div class="rounded-xl border border-surface-200 bg-white p-5">
          <div class="flex items-start justify-between gap-4 mb-4">
            <div>
              <p class="text-sm font-bold text-surface-900 mb-0.5">Transform</p>
              <p class="text-xs text-surface-500">Subtle lift for cards or scale-up for icon buttons.</p>
            </div>
            <code class="shrink-0 text-xs font-mono bg-surface-100 text-surface-700 px-2 py-1 rounded-md"
          ...
          <div class="flex flex-wrap gap-3 items-center">
            <div
              class="px-5 py-3 bg-surface-50 border border-surface-200 rounded-xl text-sm text-surface-700 cursor-pointer transition-transform duration-150 ease-out hover:-translate-y-0.5"
            >
              Card lift
            </div>
            <button
              class="w-9 h-9 flex items-center justify-center rounded-full bg-primary-100 text-primary-600 transition-transform duration-150 ease-out hover:scale-110 active:scale-95"
            >
    ...
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-2">Tailwind Keyframe Animations</h2>
      <p class="text-sm text-surface-500 mb-6">
        Built-in
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">animate-*</code>
      ...
      <div class="rounded-xl border border-surface-200 bg-white overflow-hidden divide-y divide-surface-100">
        @for (anim of keyframeAnims; track anim.token) {
          <div class="flex items-center gap-6 px-6 py-4">
            <span class="w-36 shrink-0 text-xs font-mono text-primary-600">{{ anim.token }}</span>
            <span class="w-40 shrink-0 text-xs text-surface-500">{{ anim.use }}</span>
    ...
    <!-- Angular Animations -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-2">Angular Animations</h2>
      <p class="text-sm text-surface-500 mb-6">
        Use
        ...
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">provideAnimations()</code>
        in the app config.
      </p>
        ...
        @for (example of angularExamples; track example.label) {
          <div class="rounded-xl border border-surface-200 bg-white overflow-hidden">
            <div class="px-5 py-4 border-b border-surface-100">
              <p class="text-sm font-bold text-surface-900 mb-0.5">{{ example.label }}</p>
              <p class="text-xs text-surface-500">{{ example.description }}</p>
            </div>
            <div class="bg-surface-900 px-5 py-4 overflow-x-auto">
              <pre class="font-mono text-sm leading-relaxed text-surface-100 whitespace-pre">{{ example.code }}</pre>
            </div>
          </div>
        }
      </div>
    ...
    <!-- Reduced Motion -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-surface-900 mb-2">Reduced Motion</h2>
      <p class="text-sm text-surface-500 mb-4">
        ...
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">motion-reduce:</code>
        variant or Angular's animation
        <code class="text-xs font-mono bg-surface-100 text-surface-700 px-1.5 py-0.5 rounded">disabled</code>
        state.
      </p>
      <div class="rounded-xl border border-surface-200 bg-white overflow-hidden">
        <div class="bg-surface-900 px-5 py-4 overflow-x-auto">
          <pre class="font-mono text-sm leading-relaxed text-surface-100 whitespace-pre">{{ reducedMotionCode }}</pre>
        </div>
      </div>
    </section>
  ...
  readonly keyframeAnims = [
    {
      ...
      demoClass: 'w-5 h-5 rounded-full border-2 border-surface-200 border-t-primary-500 animate-spin',
    },
    {
      token: 'animate-pulse',
      use: 'Skeleton loading placeholders',
      demoClass: 'w-32 h-4 rounded bg-surface-200 animate-pulse',
    },
    {
      token: 'animate-bounce',
      use: 'Attention cues — use very sparingly',
      demoClass: 'w-4 h-4 rounded-full bg-primary-400 animate-bounce',
    },
  ];
