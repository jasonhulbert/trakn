...
@Component({
  ...
  standalone: true,
  template: `
    <h1 class="text-3xl font-extrabold text-fore-300 tracking-tight mb-2">Spacing</h1>
    ...
    <!-- Spacing Scale -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-fore-300 mb-2">Spacing Scale</h2>
      ...
      <div class="rounded-xl border border-base-700 bg-base-800 overflow-hidden divide-y divide-base-700">
        <div class="flex items-center gap-6 px-6 py-2 bg-base-900">
          <span class="w-16 shrink-0 text-xs font-semibold text-fore-600 uppercase tracking-wider">Token</span>
          <span class="w-16 shrink-0 text-xs font-semibold text-fore-600 uppercase tracking-wider">rem</span>
          <span class="w-12 shrink-0 text-xs font-semibold text-fore-600 uppercase tracking-wider">px</span>
          <span class="text-xs font-semibold text-fore-600 uppercase tracking-wider">Visual</span>
        </div>
        @for (step of spacingScale; track step.token) {
          <div class="flex items-center gap-6 px-6 py-3">
            <span class="w-16 shrink-0 text-xs font-mono text-accent-500">{{ step.token }}</span>
            <span class="w-16 shrink-0 text-xs font-mono text-fore-600">{{ step.rem }}</span>
            <span class="w-12 shrink-0 text-xs font-mono text-fore-700">{{ step.px }}</span>
            <div class="bg-accent-400 rounded-sm" [style.width]="step.width" style="height: 14px;"></div>
          </div>
        }
      </div>
    </section>
    ...
    <!-- Border Radius Scale -->
    <section class="mb-12">
      <h2 class="text-xl font-bold text-fore-300 mb-2">Border Radius</h2>
      <p class="text-sm text-fore-600 mb-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
        @for (r of radiusScale; track r.token) {
          <div class="flex flex-col items-start gap-3">
            <div [class]="'bg-accent-500/20 border-2 border-accent-500/40 ' + r.class + ' ' + r.sizeClass"></div>
            <div>
              <p class="text-xs font-semibold text-fore-300">{{ r.token }}</p>
              <p class="text-xs font-mono text-fore-700">{{ r.px }}</p>
            </div>
          </div>
        }
      </div>
    ...
    <section class="mb-12">
      <h2 class="text-xl font-bold text-fore-300 mb-2">Common Patterns</h2>
      <p class="text-sm text-fore-600 mb-6">Spacing conventions used consistently throughout the app.</p>
      <div class="rounded-xl border border-base-700 bg-base-800 overflow-hidden divide-y divide-base-700">
        @for (pattern of commonPatterns; track pattern.label) {
          <div class="px-6 py-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-sm font-bold text-fore-300 mb-0.5">{{ pattern.label }}</p>
                <p class="text-xs text-fore-600 mb-2">{{ pattern.description }}</p>
              </div>
            </div>
            <div class="flex flex-wrap gap-2 mt-1">
              @for (token of pattern.tokens; track token.class) {
                <span
                  class="inline-flex items-center gap-1.5 text-xs font-mono bg-base-700 text-fore-300 px-2 py-1 rounded-md"
                >
                  <span class="w-2 h-2 rounded-sm bg-accent-400 shrink-0"></span>
                  {{ token.class }}
                  <span class="text-fore-700">({{ token.px }})</span>
                </span>
              }
            </div>
          </div>
